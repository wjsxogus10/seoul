...
def load_and_merge_data():
    ...
    if os.path.exists(subway_path):
        try:
            ...
            # 컬럼명 매칭 ('자치구_코드_명' -> '자치구명')
            # 파일 컬럼: ['자치구_코드_명', '면적(km²)', '지하철역_수', '지하철역_밀도(개/km²)']
            if '자치구_코드_명' in df_sub.columns and '지하철역_밀도(개/km²)' in df_sub.columns:
                df_sub = df_sub.rename(columns={'자치구_코드_명': '자치구명', '지하철역_밀도(개/km²)': '지하철역 밀도'})
                
                # 병합
                gdf = gdf.merge(df_sub[['자치구명', '지하철역 밀도', '지하철역_수']], on='자치구명', how='left')
                ...
                st.error(f"❌ 지하철 파일 컬럼 불일치: {list(df_sub.columns)}")
                gdf['지하철역 밀도'] = 0
                gdf['지하철역_수'] = 0
        except Exception as e:
            st.error(f"지하철 파일 읽기 오류: {e}")
            gdf['지하철역 밀도'] = 0
            gdf['지하철역_수'] = 0
    else:
        # 파일이 없으면 0 처리
...
if gdf is not None:
    ...
    if valid_metrics:
        ...
        st.warning("데이터 파일이 로드되지 않았습니다. data 폴더를 확인해주세요.")
else:
