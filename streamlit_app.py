...
def load_and_merge_data():
    ...
    try:
        ...
        if 'name' in gdf.columns:
            ...
            # Fatal error if we can't find the district name
            return None, None 
            
        gdf['면적(km²)'] = gdf.geometry.to_crs(epsg=5179).area / 1_000_000
    except Exception as e:
        return None, None # Crash if base map fails

    # --- 2. 컬럼 초기화 (KeyError 방지) ---
...
if result is None or result[0] is None:
    st.error("❌ 데이터를 로드하지 못했습니다. (필수 지도 또는 데이터 병합 실패)")
    st.info("💡 해결 방법: 1. GitHub 'data' 폴더 내 파일명 확인. 2. Cloud Shell에서 'streamlit cache clear' 실행 후 재부팅.")
    st.stop()
