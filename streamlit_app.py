...
def load_and_merge_data():
    ...
    # (B) 사용자 데이터 병합
    # 기본값 0으로 초기화
    cols_to_init = ['인구 밀도', '총_상주인구_수', '집객시설 수', '버스정류장 밀도', '버스정류장_수', '지하철역 밀도', '지하철역_수', '대중교통 밀도']
        ...
        try:
            ...
            if gu_col:
                ...
                if cnt_col: 
                    ...
                    cols_to_use.append('지하철역_수')
                
                df_dens = df_dens.rename(columns=rename_map)
                
                # 기존 0값 컬럼 제거 후 병합
                gdf = gdf.drop(columns=[c for c in cols_to_use if c != '자치구명' and c in gdf.columns], errors='ignore')
                gdf = gdf.merge(df_dens[cols_to_use], on='자치구명', how='left')
                
                gdf['지하철역 밀도'] = gdf['지하철역 밀도'].fillna(0)
                gdf['지하철역_수'] = gdf['지하철역_수'].fillna(0)
        except: pass

    # 5. [수정됨] 대중교통 밀도 (요청하신 공식: (버스수 + 지하철수) / 면적)
    gdf['총_교통수단_수'] = gdf['버스정류장_수'] + gdf['지하철역_수']
    gdf['대중교통 밀도'] = gdf['총_교통수단_수'] / gdf['면적(km²)']
    
    # 순위 (밀도가 높을수록 1위? 아니면 부족한 곳 찾기? 여기선 밀도로 표기)
    # 만약 '부족 순위'를 원하시면 rank ascending=True로 하면 됩니다.
    # 여기서는 '밀도' 자체를 보여줍니다.
...
with col_map:
    ...
    # [수정됨] 점 찍기(Scattermapbox) 코드 삭제됨 (요청 사항 반영)
...
with col_chart:
    ...
    # 소수점 포맷
    fmt = '%{text:.0f}' if '인구' in selected_name or '시설' in selected_name else '%{text:.4f}'
